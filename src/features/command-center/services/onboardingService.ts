...
export async function saveOnboarding(_workspaceId: string, userId: string, values: { trade: string; zip?: string; focus: string; jobSize: string; veteranOwned: boolean }) {
  ...
  return upsertDashboardBootstrap({
    ...
    profile_snapshot: values.zip ? { zip: values.zip } : undefined,
    widget_config: [],
  });
}
